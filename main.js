...
function draw(){
    ...
    if(drawn_sketch == sketch){
        answer_holder = "set"
        score++;
        document.getElementById("score_ans").innerHTML = "Score- " + score;
    }
    strokeWeight(5);
    stroke("Black");
    if (mouseIsPressed){
...
function update_canvas(){
    ...
    document.getElementById("pic_ans").innerHTML = sketch;
}

function check_sketch(){
    timer_counter++;
    document.getElementById("timer_ans").innerHTML = "Timer- "+ timer_counter;

    if(timer_counter > 2000){
        timer_counter = 0;
        timer_checked = "completed";
    }
...
function gotResult(error, results){
    if(error){
        ...
        document.getElementById("name_ans").innerHTML = results[0].label;

        document.getElementById("accuracy_ans").innerHTML = Math.round(results[0].confidence*100) + "%";
